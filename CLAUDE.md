...
- **Brindes e Permutas — Fase 1** (spec completa em `BRINDES_PERMUTAS.md`, 5 fases — só a Fase 1 concluída até
  aqui) — campo `natureza` (choices `venda`/`brinde`/`permuta`, default `venda`) em `eventos.ItemOrcamento`,
  ...
  de mercado" que o PDF vai mostrar riscado numa fase futura. Como `recalcular_totais()` dos três (`Orcamento`,
  ...
  sem endpoint novo, só payload (ver Endpoints). **Fora do escopo desta fase** (vêm nas Fases 2-5, ainda não
  implementadas): guard de valor zero em `financeiro/signals.py` (hoje um `PedidoPDV`/evento 100% brinde
  ainda geraria um `MovimentoFinanceiro` de R$0,00 — inofensivo, mas não filtrado), filtro `natureza='venda'`
  em `relatorios.ProdutosMaisVendidosView` (hoje brinde/permuta com `preco_total=0` já não contam no **valor**
  do ranking, mas ainda contam na **quantidade** — só filtrar de verdade evita isso), PDF (valor riscado +
  rótulo em `pdf_orcamento.py`/`pdf_contrato.py`), seletor de natureza no frontend (`Orcamentos.jsx`/
  `Eventos.jsx`/`PDV.jsx`) e o app avulso reaproveitando `PedidoPDV` puro (já suportado pela Fase 1, só falta
  UI). Não persegui essas fases nesta sessão — pedido explícito do usuário foi só "Fase 1"
...
| Brindes e Permutas — Fase 1 de 5 (campo `natureza` venda/brinde/permuta por item, zera `preco_total` sem faturar) | Spec completa em `BRINDES_PERMUTAS.md` (5 fases). Fase 1 (ver "Brindes e Permutas — Fase 1" em Padrões Obrigatórios): `natureza` novo em `eventos.ItemOrcamento`/`eventos.ItemEvento`/`pdv.ItemPedidoPDV`, `save()` dos três zera `preco_total` quando `natureza != 'venda'` (preserva `preco_unit` como referência de valor de tabela); `recalcular_totais()` dos três não precisou mudar (já soma sem filtro); `converter_em_evento` propaga `natureza`. 11 testes novos (`BrindesPermutasTests` em `eventos/tests.py`, `BrindesPermutasPDVTests` em `pdv/tests.py`) + suíte completa (325) verde. Faltam Fases 2-5 (guard de valor zero no Financeiro, filtro no ranking de produtos mais vendidos, PDF com valor riscado, seletor de natureza no frontend) — não implementadas nesta sessão, pedido explícito do usuário foi só a Fase 1 | 🔄 Em andamento (fase 1 de 5 — código local, ainda não commitado nem deployado) |
...
- Não esquecer de propagar `natureza=item.natureza` em `OrcamentoViewSet.converter_em_evento` ao criar cada `ItemEvento` — sem isso, todo item de brinde/permuta de um orçamento convertido vira venda cobrada no evento gerado (ver "Brindes e Permutas — Fase 1")

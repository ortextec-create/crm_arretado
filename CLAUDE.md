...
- **Brindes e Permutas — Fases 1-3** (spec completa em `BRINDES_PERMUTAS.md`, 5 fases — 1 a 3 concluídas até
  ...
  **não muda** — já não expõe preço de item nenhum, brinde/permuta aparecem normalmente na lista de produção
  (decisão do spec). **Fora do escopo ainda** (Fases 4-5, não implementadas): seletor de natureza no frontend
...
| Brindes e Permutas — Fases 1-3 de 5 (campo `natureza` venda/brinde/permuta por item, zera `preco_total` sem faturar; guard financeiro + filtro no ranking; PDF com valor riscado) | Spec completa em `BRINDES_PERMUTAS.md` (5 fases). Fase 1 (ver "Brindes e Permutas — Fases 1-3" em Padrões Obrigatórios): `natureza` novo em `eventos.ItemOrcamento`/`eventos.ItemEvento`/`pdv.ItemPedidoPDV`, `save()` dos três zera `preco_total` quando `natureza != 'venda'` (preserva `preco_unit` como referência de valor de tabela); `recalcular_totais()` dos três não precisou mudar (já soma sem filtro); `converter_em_evento` propaga `natureza`. Fase 2: guard `if pedido.total <= 0: return` em `financeiro/signals.py::_registrar_venda_pdv()` (+ guarda simétrica em `_registrar_pagamento_evento()`, defensiva); `relatorios.ProdutosMaisVendidosView._qs_pdv()`/`_qs_eventos()` ganharam `natureza='venda'` no filtro (iFood não tem o campo, nada a mudar lá). Fase 3: `pdf_orcamento.py`/`pdf_contrato.py` — nome do item ganha `" — Brinde"/" — Permuta"`, `preco_unit` riscado via `Paragraph`+`<strike>` (só na célula do item afetado); `pdf_resumo_cozinha.py` não muda (já não expõe preço). Deploy das Fases 1-2 em produção em 24/ago/2026 (commits `661cd27`/`5ab6832`, sem tag — só backend/relatório, nada usável na UI ainda). 18 testes novos ao todo (Fases 1-3) + suíte completa (332) verde. Faltam Fases 4-5 (seletor de natureza no frontend, app avulso via PDV) — pedido explícito do usuário foi implementar fase a fase | 🔄 Em andamento (fases 1-3 de 5 — código local da Fase 3 ainda não commitado nem deployado) |
...
- Não esquecer de propagar `natureza=item.natureza` em `OrcamentoViewSet.converter_em_evento` ao criar cada `ItemEvento` — sem isso, todo item de brinde/permuta de um orçamento convertido vira venda cobrada no evento gerado (ver "Brindes e Permutas — Fases 1-3")

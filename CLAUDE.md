...
- Não estender `authentication_classes`/`permission_classes` globalmente em `config/settings.py` por causa da autenticação real — cada app opta localmente, na própria classe da view (padrão: `usuarios/`, `auditoria/`, e agora `eventos.EventoViewSet`/`OrcamentoViewSet` para auditar pagamentos); os apps ainda não instrumentados continuam `AllowAny`/`CsrfExemptMixin` como antes
- Ao adicionar `TokenAuthentication` a uma viewset só para capturar o ator em auditoria, **não** assuma que isso exige login — `authentication_classes` só popula `request.user` quando o header vem; `permission_classes` (`AllowAny` vs `IsAuthenticated`) é quem decide se a ação é bloqueada sem login. Ver `EventoViewSet.get_permissions()` (só `adicionar_pagamento`/`remover_pagamento` exigem `IsAuthenticated`) vs. `OrcamentoViewSet` (continua `AllowAny` em tudo, captura o ator de forma oportunista em `converter_em_evento`)

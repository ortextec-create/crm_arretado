...
│   │                               `Evento.observacoes_cozinha` (TextField, migration 0016) — orientações internas
│   │                               da equipe de produção, só sai no PDF de resumo de cozinha, nunca client-facing
│   │                               (só no Evento, não no Orcamento) — ver "Resumo de Cozinha" e "O Que NÃO Fazer"
...
│   │                               por categoria, nunca expõe preço — ver "Resumo de Cozinha" abaixo. Inclui
│   │                               caixa destacada "ORIENTAÇÕES PARA A COZINHA" (`_callout_cozinha`) logo abaixo
│   │                               do bloco cliente/local quando `Evento.observacoes_cozinha` está preenchido —
│   │                               nunca sai em `pdf_orcamento.py`/`pdf_contrato.py` (é o único campo textual do
│   │                               Evento exclusivo do resumo de cozinha; `observacoes` continua saindo tanto
│   │                               aqui — seção "OBSERVAÇÕES GERAIS" — quanto no orçamento client-facing)
...
- **Resumo de Cozinha** (`GET /eventos/{id}/resumo-cozinha/`, `eventos/pdf_resumo_cozinha.py::gerar_pdf_resumo_cozinha(evento, imagens=None)`) — PDF operacional interno (não client-facing) com a lista de itens do Evento agrupada por categoria, pra a cozinha montar a produção. Usa ReportLab **Platypus** (não canvas cru como `pdf_orcamento.py`), porque a lista de itens tem tamanho variável e pode quebrar página — e **sem** timbre/marca d'água (`_mesclar_timbre` nunca é chamado aqui). Itens são ordenados por `produto__categoria__ordem`/`produto__categoria__nome`/`nome` na própria query e agrupados em memória com `itertools.groupby` (nunca reordenar em Python depois) — item sem `produto` ou cujo `produto` não tem `categoria` cai no grupo `"Outros"`, sempre por último (a ordenação por `ordem` já garante isso via `NULLS LAST` do Postgres, então o agrupamento em Python não precisa reordenar nada). **Nunca expõe preço** (`preco_unit`/`preco_total`/`valor_total`) — é 100% operacional. Todo texto livre (nome do cliente, endereço do local, observação do item/evento) passa por `xml.sax.saxutils.escape()` antes de virar `Paragraph`, porque a mini-sintaxe XML do ReportLab quebra a geração do PDF se o texto tiver `&`/`<`/`>` sem escapar. Endpoint é `AllowAny` (mesmo padrão de `OrcamentoViewSet.pdf`/`ContratoViewSet.pdf` — é leitura pura, não audita).
  **Orientações para a cozinha (`Evento.observacoes_cozinha`, migration 0016)**: campo de texto livre exclusivo do resumo de cozinha — quando preenchido, `_callout_cozinha()` desenha uma caixa destacada "ORIENTAÇÕES PARA A COZINHA" (fundo `#FBF0DD`, borda caramelo) logo abaixo do bloco cliente/local e antes de "ITENS". **Nunca** entra em `pdf_orcamento.py`/`pdf_contrato.py` — é o campo pra instrução interna de produção (alergias, montagem no local, cor de cobertura…), diferente de `Evento.observacoes` (que sai aqui na seção "OBSERVAÇÕES GERAIS" **e** no orçamento client-facing). Exposto em `EventoDetailSerializer`/`EventoCreateSerializer` (não no `EventoListSerializer`), auditado no PATCH via `campos_log_atualizacao`, editável em qualquer status (`EventoViewSet.update` não trava por status). Frontend: textarea "Orientações para a cozinha" no wizard de criação, no `ModalEditarEvento` e numa aba dedicada **"Cozinha"** do `ModalDetalheEvento` (ao lado de "Histórico" — textarea + botão "Salvar orientações" que faz `eventosApi.update(id, {observacoes_cozinha})`, editável em qualquer status; o botão fica `disabled` sem alteração pendente); o bloco somente-leitura `styles.cozinhaCard` (ícone `ti-tools-kitchen-2`) na sidebar do detalhe continua existindo e agora abre essa aba ao clicar. Só no Evento — não há campo equivalente no Orcamento (conversão nasce com ele vazio).
  **Imagens de inspiração (opcional, `?imagens=1`)**: quando o parâmetro vem, o PDF ganha uma folha separada no final (`PageBreak()` + título "IMAGENS DE INSPIRAÇÃO") com cada imagem de `evento.galeria_imagens_inspiracao()` (mesmo helper usado pelo `EventoDetailSerializer`, ver bloco `eventos.ImagemInspiracao`), via `platypus.Image(..., kind='proportional')` — nunca distorce, encolhe mantendo a proporção original dentro de uma caixa máxima (`IMG_MAX_W`/`IMG_MAX_H`). Sem o parâmetro (ou evento sem nenhuma imagem), o PDF sai idêntico a antes — 1 página a menos, sem a folha. Arquivo de imagem ausente/corrompido no disco é pulado (`try/except` por imagem, nunca derruba a geração do PDF inteiro). `EventoListSerializer.n_imagens_inspiracao` (contagem, via o mesmo helper) existe só pra o frontend decidir se pergunta "incluir imagens?" antes de imprimir — `EventoDetailSerializer` já expõe a lista completa (`imagens_inspiracao`), o frontend usa o `length` dela quando já tem o detalhe carregado. Botão "Imprimir resumo de cozinha" (`ti-printer`) em dois lugares no frontend — card de detalhe do Evento e linha da lista — ambos chamando `handleResumoCozinha(eventoId, temImagens)`: com imagem(ns) anexada(s), um `window.confirm()` pergunta se inclui a folha de imagens (mesmo padrão de confirmação já usado antes de enviar WhatsApp) antes de chamar `eventosApi.resumoCozinha(id, incluirImagens)` (blob) e abrir com `window.open(url, '_blank')`, mesmo padrão de `handlePdf`/`handleVerPdf` já usado pros outros PDFs do sistema
...
| Resumo de Cozinha (Evento) | PDF operacional (A4 página cheia, ReportLab Platypus, sem timbre) com itens do Evento agrupados por categoria, pra a equipe de cozinha montar a produção — sem preços. Opção de incluir as imagens de inspiração numa folha separada no final (`?imagens=1`, com confirmação no frontend, 05/set/2026). Campo `Evento.observacoes_cozinha` (migration 0016, 10/set/2026) — orientações internas de produção numa caixa destacada no PDF, editável no wizard de criação e no modal de edição, nunca em documento client-facing. Botão em `Eventos.jsx` (card de detalhe + linha da lista) | ✅ Concluída (só A4 página cheia — meia-folha/térmica fora de escopo por ora) |
...
GET                   /api/v1/eventos/{id}/resumo-cozinha/               ← AllowAny · PDF operacional (itens agrupados por categoria, sem preço) · caixa "ORIENTAÇÕES PARA A COZINHA" quando Evento.observacoes_cozinha preenchido · ?imagens=1 inclui folha separada com as imagens de inspiração · ver "Resumo de Cozinha"
...
- Não incluir `Evento.observacoes_cozinha` em nenhum documento client-facing (`pdf_orcamento.py`/`pdf_contrato.py`/WhatsApp) — é orientação interna de produção, sai só no resumo de cozinha (`_callout_cozinha`). `Evento.observacoes` continua sendo o campo que aparece nos dois lugares. Não criar campo equivalente no `Orcamento` sem pedir — decisão consciente de ficar só no Evento

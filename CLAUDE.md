...
- **Brindes e Permutas — Fases 1-2** (spec completa em `BRINDES_PERMUTAS.md`, 5 fases — 1 e 2 concluídas até
  ...
  **não mudou** — `ItemPedidoIFood` não tem campo `natureza` (o conceito de brinde/permuta não existe no
  iFood, canal externo), nada a filtrar ali. **Fora do escopo ainda** (Fases 3-5, não implementadas): PDF
  (valor riscado + rótulo em `pdf_orcamento.py`/`pdf_contrato.py`), seletor de natureza no frontend
  (`Orcamentos.jsx`/`Eventos.jsx`/`PDV.jsx`) e o app avulso reaproveitando `PedidoPDV` puro (já suportado
  pelo backend desde a Fase 1, só falta UI)
...
| Brindes e Permutas — Fases 1-2 de 5 (campo `natureza` venda/brinde/permuta por item, zera `preco_total` sem faturar; guard financeiro + filtro no ranking) | Spec completa em `BRINDES_PERMUTAS.md` (5 fases). Fase 1 (ver "Brindes e Permutas — Fases 1-2" em Padrões Obrigatórios): `natureza` novo em `eventos.ItemOrcamento`/`eventos.ItemEvento`/`pdv.ItemPedidoPDV`, `save()` dos três zera `preco_total` quando `natureza != 'venda'` (preserva `preco_unit` como referência de valor de tabela); `recalcular_totais()` dos três não precisou mudar (já soma sem filtro); `converter_em_evento` propaga `natureza`. Deploy da Fase 1 em produção em 24/ago/2026 (commit `661cd27`, sem tag — só backend, nada usável na UI ainda). Fase 2 (mesma sessão): guard `if pedido.total <= 0: return` em `financeiro/signals.py::_registrar_venda_pdv()` (+ guarda simétrica em `_registrar_pagamento_evento()`, defensiva); `relatorios.ProdutosMaisVendidosView._qs_pdv()`/`_qs_eventos()` ganharam `natureza='venda'` no filtro (iFood não tem o campo, nada a mudar lá). 16 testes novos ao todo (Fases 1+2) + suíte completa (330) verde. Faltam Fases 3-5 (PDF com valor riscado, seletor de natureza no frontend, app avulso via PDV) — pedido explícito do usuário foi implementar fase a fase | 🔄 Em andamento (fases 1-2 de 5 — código local da Fase 2 ainda não commitado nem deployado) |
...
- Não esquecer de propagar `natureza=item.natureza` em `OrcamentoViewSet.converter_em_evento` ao criar cada `ItemEvento` — sem isso, todo item de brinde/permuta de um orçamento convertido vira venda cobrada no evento gerado (ver "Brindes e Permutas — Fases 1-2")

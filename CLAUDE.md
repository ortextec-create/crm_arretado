...
│   │                               em andamento — fases 0-4 de 8 concluídas, ver Pendências)
...
│                                    ContaReceberViewSet (baixa/resumo — resumo inclui saldo de Evento via
│                                    query dinâmica, nunca materializado como linha), DespesaRecorrenteViewSet
│                                    (sem DELETE — pausar via ativo=False)
└── manage.py
...
| Módulo Financeiro — Fases 0-4 (bug fix pré-requisito, models base, `MovimentoFinanceiro.registrar()`, `ContaPagar` + baixa/cancelar/resumo, `DespesaRecorrente` + crons, `ContaReceber` + signals PDV/iFood/PagamentoEvento) | Spec completa em `FINANCEIRO.md` (9 fases, 0-8). App `financeiro/`: `CategoriaFinanceira`/`ContaBancaria`/`Fornecedor`/`ConfiguracaoFinanceira`/`TelefoneAlertaFinanceiro`, ledger `MovimentoFinanceiro` (mesmo contrato de `MovimentoEstoque`), `ContaPagar`/`ContaReceber` (obrigação projetada, `valor_pago`/`valor_recebido`/`status` derivados), `DespesaRecorrente` + `AlertaFinanceiroEnviado` + crons `gerar_contas_recorrentes`/`alertar_vencimentos`, signals de venda (PDV/iFood/PagamentoEvento) batendo automaticamente no ledger com estorno em cancelamento | 🔄 Em andamento (fases 0-4 de 8 — faltam integração com nota fiscal, fluxo de caixa e frontend) |

...
- **Brindes e Permutas — Fases 1-4** (spec completa em `BRINDES_PERMUTAS.md`, 5 fases — 1 a 4 concluídas até
  ...
  **Bug real encontrado e corrigido nesta fase**: `OrcamentoCreateSerializer.create()` e
  ...
  automatizados que só cobriam os métodos de item isolados. **Fora do escopo ainda** (Fase 5, não
  implementada): nenhuma tela nova — a Fase 5 do spec ("PDV Avulso") já está coberta pelo backend desde a
  Fase 1 e pelo frontend desde esta Fase 4 (reaproveita o mesmo `PDV.jsx`), então pode não sobrar trabalho
  nenhum quando for revisitada — só confirmar
...
- Não esquecer de propagar `natureza=item.natureza` em `OrcamentoViewSet.converter_em_evento` ao criar cada `ItemEvento` — sem isso, todo item de brinde/permuta de um orçamento convertido vira venda cobrada no evento gerado (ver "Brindes e Permutas — Fases 1-4")
